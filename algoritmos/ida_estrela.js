export function ida_estrela(n, fichas) {
    ...
    let fechados = []; //lista de fechados
        ...
        if(patamar === patamarOld && attPatamar) { 
            ...
            if(teste) {
                ...
                for(let i=0; i<abertos.length; i++) { 
                    if(abertos[i].f>patamar) {
                        descartados.push({estado: abertos[i].estado, patamar: abertos[i].f});
                        abertos.splice(i,1);
                        i--; //ajusta vetor, pq retirou um elemento, ou seja length de abertos diminiu
                    }
                }
